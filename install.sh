...
if [ -n "$1" ]; then
	...
	*) echo $usage ; exit 1 ;;
	...
	CUR_TROPRIV_SHA256='76a9ecfa57c5ac822c0154c5e5e08ff9f95e0282084cb716c7dcfe453af1c27f'
	hash sha256 2>/dev/null \
	|| err 'need sha256 to check trop.conf and tropriv.sh'
	tropconf_sha256=`sha256 -q trop.conf`
	tropriv_sha256=`sha256 -q tropriv.sh`
	[ "$tropconf_sha256" != "$CUR_TROPCONF_SHA256" ] \
	&& err 'trop.conf changed, cannot check for updates'
	[ "$tropriv_sha256" != "$CUR_TROPRIV_SHA256" ] \
	&& err 'tropriv.sh changed, cannot check for updates'
	[ ! -e ${PREFIX}/.cache ] && mkdir ${PREFIX}/.cache
	[ ! -e ${PREFIX}/.cache/conf_chksum ] \
	&& echo "$tropconf_sha256" > ${PREFIX}/.cache/conf_chksum
	[ ! -e ${PREFIX}/.cache/priv_chksum ] \
	&& echo "$tropriv_sha256" > ${PREFIX}/.cache/priv_chksum
	[ "$(cat ${PREFIX}/.cache/conf_chksum)" != "$tropconf_sha256" ] \
	&& echo 'trop.conf changed, please update this file manually' \
	&& echo "$tropconf_sha256" > ${PREFIX}/.cache/conf_chksum
	[ "$(cat ${PREFIX}/.cache/priv_chksum)" != "$tropriv_sha256" ] \
	&& echo 'tropriv.sh changed, please update this file manually' \
	&& echo "$tropriv_sha256" > ${PREFIX}/.cache/priv_chksum
	...
	if ! diff -q ${tmpfile} trop.1 >/dev/null; then
		...
		su -m root -c \
		"
		install -g 0 -o 0 -m 0640 trop.1 /usr/local/man/man1/ \
		&& gzip /usr/local/man/man1/trop.1                    \
		|| exit 1
...
[ -e "${PREFIX}" ] && { [ ! -d "${PREFIX}" ]                          \
&& err "PREFIX \`${PREFIX}' is not a directory!"                      \
|| err "PREFIX path already exists. To be safe, manually remove this" \
       "directory and restart install.sh" ;}
eval $(stat -qs ${PREFIX%/*})
[ -z "$st_uid" ] && err 'bad path for PREFIX'
[ $(id -u) -ne $st_uid ] \
&& err "please login to \`$(id -un $st_uid)' to install trop"
...
install -p -m 0770 tropriv.sh trop.conf                  ${PREFIX} || \
err 'failed to install files'
